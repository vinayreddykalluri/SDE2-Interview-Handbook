...
def main() -> None:
    ...
    check_placeholders()
    chapter_files = [p for p in all_md if p.parent.name.startswith("volume-01") or p.name in {"index.md", "getting-started.md", "study-plan.md"}]
    required = [p for p in chapter_files if p.parent.name == "volume-01-java-fundamentals" and p.name.endswith(".md") and p.name not in {"index.md", "chapter-template.md"}]
    check_required_sections(required)
    check_mermaid_blocks()
    # Keep placeholder check len intentionally strict only for core and index files.
