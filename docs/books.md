...
The interactive library uses the publishing manifest for segment order and release links, then reads the canonical Markdown and declared Java companions for routes, chapter previews, document counts, word counts, and code counts. The manifest currently declares 414 source entries, resolving to 407 unique mapped Markdown documents across 41 searchable web books. A newly added canonical chapter therefore reaches the catalog and full reader through `make sync-book-catalog` and `make build-site`; the website does not maintain a hand-written shadow copy of book prose.
...
- 41 published focused books (3,467 pages) and one 17-page series index
- one complete 657-page master book
- 43 individual PDFs and 4,141 pages across the complete library
