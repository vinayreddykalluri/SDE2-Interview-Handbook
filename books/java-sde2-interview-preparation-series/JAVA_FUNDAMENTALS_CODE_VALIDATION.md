...
Canonical companion: `series/volumes/03-java-foundations-for-problem-solving/code/JavaFundamentalsExamples.java`

## Validation strategy

The existing publishing pipeline was not replaced. Volume 03 now uses the existing `code_companion` mechanism in `series/series.json`, so the full Java file appears in the PDF and remains editable/compilable as a standalone source.
