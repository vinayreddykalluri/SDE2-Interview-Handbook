...
17. **Foundation:** Draw `previous`, `current`, and saved `next` during every step of reversing `1->2->3`.
18. **Interview Core:** Reverse a validated one-based sublist using a sentinel; an invalid range must not partially mutate input.
19. **Interview Core:** Remove the nth node from the end with a measured two-pointer gap and handle head removal uniformly.
20. **Interview Core:** Return cycle entry and list intersection by node identity, not value. State the acyclic assumption for intersection.
21. **Interview Core:** Merge two disjoint sorted lists by relinking nodes and define a stable equality policy.
22. **SDE-2 Follow-up:** Deep-copy a random-pointer list by weaving, then verify every copied pointer targets a copy and the original list is restored.
