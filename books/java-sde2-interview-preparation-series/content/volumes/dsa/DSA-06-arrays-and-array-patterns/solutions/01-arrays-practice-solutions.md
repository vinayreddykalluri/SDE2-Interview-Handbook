...
## G. Sorting and Selection Solutions

19. Save the current value before shifting. The states are `[4|2,2,3]`, `[2,4|2,3]`, `[2,2,4|3]`, and `[2,2,3,4]`. Shift only while `previous > current`. With `>=`, the second `2` moves before the first and stability is lost. The invariant is that the prefix before the divider is sorted and stable.

20. Split half-open ranges until their size is below two, then merge into one reusable buffer. At equal keys, take from the left half:

```java
if (rightIndex == right
        || leftIndex < middle
        && left[leftIndex].score() <= right[rightIndex].score()) {
    // copy the left item
}
```

Use distinct labels or `originalPosition` in the test: sorting `[(2,0),(1,1),(2,2)]` must yield `[(1,1),(2,0),(2,2)]`. Key-only assertions cannot prove stability. Time is `O(n log n)` and the reusable buffer is `O(n)`.

21. Maintain `[left,lower)` less than pivot, `[lower,scan)` equal, `[scan,upper]` unknown, and `(upper,right]` greater. A less value swaps with `lower` and advances both indexes; equality advances `scan`; a greater value swaps with `upper` and decreases only `upper`, because the incoming value is unclassified. Recurse outside the settled equal region. All-equal input finishes one partition with no recursive subproblem. See `threeWayQuickSort` in the companion for the complete implementation.

22. Compute `width = (long) maximum - minimum + 1`; reject `minimum > maximum`, a policy-exceeding width, and every input outside the range. Count at offset `value - minimum`, then emit `minimum + offset` frequency times. This is `O(n+k)` time and `O(k)` space; it is inappropriate when `k` is large relative to `n`. `countingSort` contains the validated implementation.

23. Validate `0 <= k < n`, clone the array, and keep inclusive active bounds. Randomly choose a pivot, partition, and compare its settled index with `k`; continue left or right in a loop. Return only when the rank settles. The clone is the price of the preservation contract. Expected time is `O(n)`, worst-case `O(n^2)`, and iteration avoids selection call depth.

24. For hundreds of deterministic random arrays, clone the same input for the implementation and for `Arrays.sort`, then compare complete results. For nonempty inputs, check every rank against the sorted clone and verify the source is unchanged. Include duplicate-heavy values by drawing from a small numeric range. A fixed seed makes a failure replayable; it samples rather than exhaustively proves the input space, so targeted boundary tests and invariant reasoning remain necessary.
