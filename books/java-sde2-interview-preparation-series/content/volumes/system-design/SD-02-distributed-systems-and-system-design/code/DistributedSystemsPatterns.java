...
public final class DistributedSystemsPatterns {
    ...
    /**
     * Models the unique effect identity owned by one consumer. In a real
     * service, insertion into this ledger and the business update belong in
     * the same local database transaction.
     */
    public static final class IdempotentEffectLedger {
        private final Map<String, String> fingerprints = new HashMap<>();

        public synchronized ApplyResult apply(
                String eventId, String payloadFingerprint) {
            requireText(eventId, "eventId");
            requireText(payloadFingerprint, "payloadFingerprint");
            String existing = fingerprints.putIfAbsent(
                    eventId, payloadFingerprint);
            if (existing == null) {
                return ApplyResult.APPLIED;
            }
            return existing.equals(payloadFingerprint)
    ...
    public record CacheEntry<T>(T value, long version,
                                long expiresAtNanos) {
        public CacheEntry {
            Objects.requireNonNull(value, "value");
            if (version < 0 || expiresAtNanos < 0) {
                throw new IllegalArgumentException(
                        "cache version and expiry must be nonnegative");
            }
        }
    }
    ...
    /**
     * A version requirement is a correctness boundary: stale-on-error may
     * serve an expired entry, but never one older than the caller's required
     * version.
     */
    public static CacheDecision decideCacheRead(
            CacheEntry<?> entry,
            long requiredVersion,
            long nowNanos,
            boolean originAvailable,
            boolean allowExpiredOnError) {
        if (requiredVersion < 0 || nowNanos < 0) {
            throw new IllegalArgumentException(
                    "requiredVersion and time must be nonnegative");
        }
        boolean meetsVersion = entry != null
                && entry.version() >= requiredVersion;
        boolean unexpired = entry != null
                && nowNanos < entry.expiresAtNanos();
        if (meetsVersion && unexpired) {
            return CacheDecision.SERVE_FRESH;
        }
        if (originAvailable) {
            return CacheDecision.REFRESH_FROM_ORIGIN;
        }
        if (meetsVersion && allowExpiredOnError) {
            return CacheDecision.SERVE_STALE;
        }
        return CacheDecision.FAIL_CLOSED;
    }

    /**
     * Rendezvous hashing: every key selects the node with the highest stable
     * key/node score. Adding one node moves a key only when that node wins.
     */
    public static String rendezvousNode(String key, List<String> nodes) {
        requireText(key, "key");
        ...
        for (String node : nodes) {
            ...
            long score = stableScore(key, node);
            if (winner == null
                    || Long.compareUnsigned(score, winningScore) > 0) {
                winner = node;
                winningScore = score;
            }
        }
    ...
    public record AdmissionSnapshot(
            int queued,
            int queueCapacity,
            Duration oldestWorkAge,
            boolean dependencyHealthy) {
        public AdmissionSnapshot {
            if (queued < 0 || queueCapacity < 1 || queued > queueCapacity) {
                throw new IllegalArgumentException("invalid queue occupancy");
            }
            Objects.requireNonNull(oldestWorkAge, "oldestWorkAge");
            if (oldestWorkAge.isNegative()) {
                throw new IllegalArgumentException("negative oldest-work age");
            }
        }
    }
    ...
    public enum AdmissionDecision {
        ACCEPT,
        REJECT_DEPENDENCY_UNHEALTHY,
        REJECT_STALE_BACKLOG,
        REJECT_QUEUE_FULL
    }

    /** Rejects work before an already unhealthy queue consumes more memory. */
    public static AdmissionDecision decideAdmission(
            AdmissionSnapshot snapshot, Duration maximumUsefulQueueAge) {
        Objects.requireNonNull(snapshot, "snapshot");
        requirePositive(maximumUsefulQueueAge, "maximumUsefulQueueAge");
        if (!snapshot.dependencyHealthy()) {
            return AdmissionDecision.REJECT_DEPENDENCY_UNHEALTHY;
        }
        if (snapshot.oldestWorkAge().compareTo(maximumUsefulQueueAge) >= 0) {
            return AdmissionDecision.REJECT_STALE_BACKLOG;
        }
        if (snapshot.queued() == snapshot.queueCapacity()) {
    ...
    public static void main(String[] args) {
        ...
        effectLedgerAssertions();
        cacheAssertions();
        rendezvousAssertions();
        admissionAssertions();
        System.out.println("PASS distributed-systems executable checks");
    }
    ...
    private static void effectLedgerAssertions() {
        var ledger = new IdempotentEffectLedger();
        assert ledger.apply("event-1", "amount=10") == ApplyResult.APPLIED;
        assert ledger.apply("event-1", "amount=10") == ApplyResult.DUPLICATE;
        assert ledger.apply("event-1", "amount=11") == ApplyResult.CONFLICT;
        assert ledger.appliedCount() == 1;
        assert ledger.apply("event-2", "amount=10") == ApplyResult.APPLIED;
        assert ledger.appliedCount() == 2;
    }

    private static void cacheAssertions() {
        var fresh = new CacheEntry<>("v7", 7, 200);
        assert decideCacheRead(fresh, 7, 100, true, false)
                == CacheDecision.SERVE_FRESH;
        assert decideCacheRead(fresh, 8, 100, true, true)
                == CacheDecision.REFRESH_FROM_ORIGIN;
        assert decideCacheRead(fresh, 7, 250, false, true)
                == CacheDecision.SERVE_STALE;
        assert decideCacheRead(fresh, 8, 250, false, true)
                == CacheDecision.FAIL_CLOSED;
        assert decideCacheRead(null, 0, 100, true, false)
                == CacheDecision.REFRESH_FROM_ORIGIN;
    }

    private static void rendezvousAssertions() {
        List<String> original = List.of("node-a", "node-b", "node-c");
        List<String> expanded = List.of(
                "node-a", "node-b", "node-c", "node-d");
        int moved = 0;
        for (int index = 0; index < 500; index++) {
            String key = "customer-" + index;
            String before = rendezvousNode(key, original);
            String after = rendezvousNode(key, expanded);
            if (!before.equals(after)) {
                moved++;
                assert after.equals("node-d")
                        : "a key moved between existing nodes";
            }
        }
        assert moved > 0 && moved < 250;
        assert rendezvousNode("stable", original)
                .equals(rendezvousNode("stable", original));
    }

    private static void admissionAssertions() {
        Duration ageLimit = Duration.ofSeconds(2);
        assert decideAdmission(new AdmissionSnapshot(
                2, 10, Duration.ofMillis(50), true), ageLimit)
                == AdmissionDecision.ACCEPT;
        assert decideAdmission(new AdmissionSnapshot(
                10, 10, Duration.ofMillis(50), true), ageLimit)
                == AdmissionDecision.REJECT_QUEUE_FULL;
        assert decideAdmission(new AdmissionSnapshot(
                4, 10, Duration.ofSeconds(3), true), ageLimit)
                == AdmissionDecision.REJECT_STALE_BACKLOG;
        assert decideAdmission(new AdmissionSnapshot(
                0, 10, Duration.ZERO, false), ageLimit)
                == AdmissionDecision.REJECT_DEPENDENCY_UNHEALTHY;
    }
    ...
    private static long stableScore(String key, String node) {
        // FNV-1a is used only as a deterministic teaching hash, not security.
        long hash = 0xcbf29ce484222325L;
        String combined = key + '\u0000' + node;
        for (int index = 0; index < combined.length(); index++) {
            hash ^= combined.charAt(index);
            hash *= 0x100000001b3L;
        }
        return hash;
    }
