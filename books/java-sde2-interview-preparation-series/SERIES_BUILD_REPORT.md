...
Exact byte counts and SHA-256 hashes are recorded in `series/dist/manifest.json`.
...
Focused evidence is recorded in the Java Fundamentals, Time/Space, Bit Manipulation, and Loop Mastery audit, coverage, validation, changelog, and build reports. Machine-readable release metadata is in `series/dist/manifest.json`.
